...
import { Loader2, MapPin, Calendar, Clock, Download, Plane, Users, Star, Globe, TrendingUp, ChevronDown, ChevronUp, FileText } from "lucide-react";
...
const Home = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({ destination: destination.trim(), numberOfDays: Number(numberOfDays) });
  };
    ...
    {[
      ...
      >
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(135deg, rgba(0,0,0,0.3), rgba(0,0,0,0.1)), url('${
              city.name === 'Paris'
                ? city.image
                : `https://images.unsplash.com/${city.image}?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80`
            }')`
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
          <div className="font-bold text-lg group-hover:text-orange-300 transition-colors">
            {city.name}
          </div>
